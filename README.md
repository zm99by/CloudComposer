...
jobs:
  upload-dags:
    ...
    - name: Upload DAGs to Cloud Storage
      run: |
        gsutil cp -r ./dags gs://${{ env.BUCKET_NAME }}/dags/

    - name: Update Cloud Composer environment
      run: |
        gcloud composer environments update <environment_name> --location ${{ env.REGION }} --update-dags
